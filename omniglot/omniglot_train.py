    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%5000 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                sample_images,sample_labels = sample_dataloader.__iter__().next()  # [5, 1, 28, 28]
                test_images,test_labels = test_dataloader.__iter__().next()  # [5, 1, 28, 28]  选取5张作为Q验证

                # calculate features
                sample_features = feature_encoder(Variable(sample_images).cuda(GPU)) # [5, 64, 5, 5]
                test_features = feature_encoder(Variable(test_images).cuda(GPU)) # [5, 64, 5, 5]
